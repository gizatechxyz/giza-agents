...
class Pool:
    def __init__(self, address, sender, token0=None, token1=None, fee=None):
        self.contract = Contract(address, abi=os.path.join(os.path.dirname(__file__), "ASSETS/pool.json"))
        self.sender = sender
        self.token0 = Contract(self.contract.token0(), abi=os.path.join(os.path.dirname(__file__), "ASSETS/erc20.json"))
        self.token0_decimals = self.token0.decimals()
        self.token1 = Contract(self.contract.token1(), abi=os.path.join(os.path.dirname(__file__), "ASSETS/erc20.json"))
    ...
    def get_pool_price(self, block_number=None, invert=False):
        ...
        return tick_to_price(tick, self.token0_decimals, self.token1_decimals, invert=invert)
